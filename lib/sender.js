    ...
    post_req = https.request(post_options, function (res) {
        ...
        res.on('end', function () {
            if (statusCode === 503) {
                ...
                return callback(new Error('503 Service Unavailable'));
            } else if(statusCode == 401){
        console.log('Unauthorized');
                return callback(new Error('401 Unauthorized'), null);
            } else if (statusCode !== 200) {
                var error = statusCode + ' (' + buf + ')';
                console.log(error);
            ...
            try {
                ...
                callback(new Error("GCM response exception: "+ e.toString), null);
            }
        });
    });
    ...
    if (registrationId.length === 1) {
        ...
        this.sendNoRetry(message, registrationId, function lambda(err, result) {
            ...
            if (result === undefined) {
                if (attempt < retries) {
                    ...
                    console.log('Could not send message after ' + retries + ' attempts', err);
                    callback(err, result);
                }
                attempt += 1;
            } else {
        ...
        this.sendNoRetry(message, registrationId, function lambda(err, result) {
            ...
            if (attempt < retries) {
                ...
                console.log('Could not send message to all devices after ' + retries + ' attempts',err);
                callback(err, result);
            }
        });
    } else {
